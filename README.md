...
Polaris 是一款常驻系统托盘的 Windows 径向应用启动器（Radial Launcher）。按住触发键即可在鼠标周围弹出一个环形面板，快速启动你常用的应用、文件夹与系统位置，松开触发键即收起。它基于 .NET 9 + WPF 构建，强调流畅的动画与高度可定制的外观。

## 功能特性

- **径向快速启动**：按住触发键唤出环形面板，点击图标即可启动对应项目。
- **多主题**：
  - **土星环（saturn）**：以行星 + 多层圆环的方式排布图标，自带光晕与旋转动画。
  - **玻璃（liquidglass）**：半透明的圆角「液态玻璃」面板，支持拖拽排序及相邻图标「让位」动画。
...
- **高度可定制外观**：面板透明度、背景色、强调色、字体颜色、图标尺寸、单环最大图标数、内环图标数等均可调节，且每个主题各自记忆自己的外观。
- **全局触发键**：默认使用右 Alt（虚拟键 `0xA5`）作为「按住显示」的触发键，可在设置中修改。
- **开机自启**：可选随 Windows 启动（默认开启），通过注册表管理。
- **单实例运行**：通过全局 Mutex 防止重复启动。
- **托盘常驻**：以系统托盘图标方式运行，包含全局异常保护，避免意外崩溃后静默退出。
...
3. 点击图标启动对应应用 / 文件夹 / 系统位置。
4. **松开触发键**收起面板。
5. 右键点击托盘图标可打开**设置**窗口，在此添加 / 删除启动项、切换主题、调整外观与触发键、配置开机自启等。
